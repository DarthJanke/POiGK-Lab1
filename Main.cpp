...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		textures.push_back(LoadTexture("spaceship1.png"));
		textures.push_back(LoadTexture("spaceship2.png"));
	...
	void Draw() const override {
		...
		Texture2D currentTexture = textures[currentTextureIndex];

		Vector2 dstPos = {
			transform.position.x - (currentTexture.width * scale) * 0.5f,
			transform.position.y - (currentTexture.height * scale) * 0.5f
		};
		DrawTextureEx(currentTexture, dstPos, 0.0f, scale, WHITE);
	}

	float GetRadius() const override {
		Texture2D currentTexture = textures[currentTextureIndex];
		return (currentTexture.width * scale) * 0.5f;
	}
	...
	void PrevTexture() {
		currentTextureIndex = (currentTextureIndex - 1 + static_cast<int>(textures.size())) % static_cast<int>(textures.size());
	}
